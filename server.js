...
app.post('/api/admin/topup', isAdmin, async (req, res) => {
    ...
    if(user) {
        // SYNC: Find user's socket and update their balance instantly
...
app.post('/api/admin/ticket/reply', isAdmin, async (req, res) => {
    ...
    const ticket = await Ticket.findByIdAndUpdate(id, { 
        adminReply: reply,
        status: 'open' // Keep open or change to 'answered'
    }, { new: true });
    
    // SYNC: Notify user if online (Optional, nice to have)
...
io.on('connection', (socket) => {
    ...
    socket.on('chat_msg', (msg) => {
        ...
        const chatMsg = { username: user.username, text: msg }; // Removed time for simplicity in sync
    ...
    socket.on('duel_respond', async (response) => {
        ...
        if (response === 'DECLINE') {
            ...
            STATE.duel.match = {
                ...
                pot: bet * 2,
                round: 1,
                target: parseInt(STATE.duel.proposal.rounds), // e.g. 1, 3, 5
...
async function runDuelMatch(gameType) {
    // Simple 50/50 logic for sync demonstration
    setTimeout(async () => {
        ...
        if (winSeat) {
            ...
            io.to(winSeat.socketId).emit('balance_update', (await User.findOne({username:winSeat.username})).credits);
        }
